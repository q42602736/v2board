...
class OrderController extends Controller
{
    ...
    public function save(OrderSave $request)
    {
        ...
        // 添加购买限制检查（将在订单创建事务中一起处理）
        $purchaseLimitService = new PurchaseLimitService();
        $eligibility = $purchaseLimitService->checkPurchaseEligibility(
            $request->user['id'],
            $request->input('plan_id')
        );

        if (!$eligibility['eligible']) {
            if ($eligibility['reason'] === 'limit_exceeded_in_period') {
                $expiredDate = date('Y-m-d H:i:s', $eligibility['expired_at']);
                $message = "该套餐在有效期内限购 {$eligibility['limit_count']} 次，您已购买 {$eligibility['purchased_count']} 次。套餐到期时间：{$expiredDate}，到期后可重新购买。";
            } else {
                $message = "该套餐购买受限，请联系客服";
            }
            abort(500, $message);
        }
        ...
        // 锁定用户记录，防止并发购买绕过限制检查
        $lockedUser = User::where('id', $request->user['id'])->lockForUpdate()->first();
